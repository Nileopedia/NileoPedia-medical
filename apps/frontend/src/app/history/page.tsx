...
import { Search, ExternalLink, Trash2, ChevronLeft, ChevronRight, Bookmark, Loader2 } from 'lucide-react';
...
export default function HistoryPage() {
  ...
  const fetchHistory = async () => {
    setLoading(true);
    try {
      const result = await api.getHistory({ page, limit: 10, category: categoryFilter || undefined, startDate: dateFilter || undefined });
  ...
  const handleDelete = async (queryId: string) => {
    ...
    try {
      ...
      addToast({ type: 'success', title: 'Query deleted' });
    } catch {
      addToast({ type: 'error', title: 'Failed to delete query' });
    }
  };
  ...
  const handleSave = async (queryId: string) => {
    try {
      ...
      addToast({ type: 'error', title: 'Failed to save response' });
    }
  };
          ...
          <CardHeader>
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              ...
              <select
                ...
                className="px-3 py-2 border border-border rounded-lg bg-input text-xs sm:text-sm text-foreground"
              >
              ...
              <input
                ...
                className="px-3 py-2 border border-border rounded-lg bg-input text-xs sm:text-sm text-foreground placeholder:text-muted-foreground"
              />
            </div>
          </CardHeader>
          <CardContent>
            {loading ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-6">No queries found</p>
            ) : (
              <>
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {filteredQueries.map((query) => (
                      <TableRow key={query.id}>
                        <TableCell className="font-medium text-foreground max-w-xs truncate">{query.question}</TableCell>
                        ...
                        <TableCell className="text-right space-x-1.5 sm:space-x-2">
                          <button className="text-primary hover:text-primary/80" title="View details" onClick={() => router.push(`/history/${query.id}`)}>
