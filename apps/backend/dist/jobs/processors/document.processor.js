...
exports.processDocumentIngestion = exports.queuePendingDocuments = exports.refreshKnowledgeBase = void 0;
...
async function queuePendingDocuments() {
    const pending = await prisma_1.default.medicalDocument.findMany({
        where: {
            ingestionStatus: {
                in: ['PENDING', 'FAILED'],
            },
    ...
    for (const doc of docsWithFiles) {
        try {
            ...
            logger_1.logger.error(`Failed to queue ingestion job for document ${doc.id}:`, error);
        }
    }
    if (queued > 0) {
        logger_1.logger.info(`Queued ${queued} documents for ingestion`);
    }
    return queued;
}
exports.queuePendingDocuments = queuePendingDocuments;
