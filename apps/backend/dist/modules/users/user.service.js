...
class UserService {
    ...
    async updatePreferences(userId, data) {
        const existing = await prisma_1.default.userPreferences.findUnique({
            where: { userId },
        });
        if (existing) {
            return prisma_1.default.userPreferences.update({
                where: { userId },
                data,
            });
        }
        return prisma_1.default.userPreferences.create({
            data: {
                userId,
