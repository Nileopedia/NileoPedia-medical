...
exports.evaluationDataset = exports.EvaluationDataset = void 0;
class EvaluationDataset {
    ...
    async evaluateResponse(questionId, retrievedDocuments, actualCitations, actualAnswer) {
    ...
    async runEvaluation() {
        ...
        for (const question of this.questions) {
            const result = await this.evaluateResponse(question.id, question.expectedRetrievedDocuments, question.expectedCitations, question.expectedAnswer);
            results.push(result);
        }
        const totalQuestions = results.length;
        const average = (results) => results.reduce((sum, r) => sum + r, 0) / totalQuestions;
        return {
            totalQuestions,
    ...
    calculatePrecision(retrieved, expected) {
        if (retrieved.length === 0)
            return 0;
        const relevant = retrieved.filter(r => expected.some(e => r.toLowerCase().includes(e.toLowerCase()) || e.toLowerCase().includes(r.toLowerCase())));
        return relevant.length / retrieved.length;
    }
    calculateRecall(retrieved, expected) {
        if (expected.length === 0)
            return 1;
        const relevant = expected.filter(e => retrieved.some(r => r.toLowerCase().includes(e.toLowerCase()) || e.toLowerCase().includes(r.toLowerCase())));
    ...
    calculateNDCG(retrieved, expected) {
        const dcg = retrieved.reduce((sum, doc, i) => {
            const isRelevant = expected.some(e => doc.toLowerCase().includes(e.toLowerCase()) || e.toLowerCase().includes(doc.toLowerCase()));
    ...
    calculateContextPrecision(retrieved, diseases) {
        if (retrieved.length === 0)
            return 0;
        if (diseases.length === 0)
            return 1;
        const relevant = retrieved.filter(r => diseases.some(d => r.toLowerCase().includes(d.toLowerCase()) || d.toLowerCase().includes(r.toLowerCase())));
        return relevant.length / retrieved.length;
    }
    calculateCitationAccuracy(actual, expected) {
        if (expected.length === 0)
            return 1;
        if (actual.length === 0)
            return 0;
        const matched = actual.filter(a => expected.some(e => a.toLowerCase().includes(e.toLowerCase()) || e.toLowerCase().includes(a.toLowerCase())));
        return matched.length / expected.length;
    }
    calculateAnswerCorrectness(actual, expected) {
        if (!actual || !expected)
            return 0;
...
exports.evaluationDataset = new EvaluationDataset();
//# sourceMappingURL=evaluation-dataset.service.js.map