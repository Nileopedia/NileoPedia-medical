...
exports.validateInput = exports.applySecurityMiddleware = void 0;
...
function inputSanitization() {
    return (req, res, next) => {
        if (req.body) {
            req.body = (0, input_validation_service_1.sanitize)(req.body);
        }
        if (req.query) {
            req.query = (0, input_validation_service_1.sanitize)(req.query);
        }
        if (req.params) {
            req.params = (0, input_validation_service_1.sanitize)(req.params);
        }
        next();
    };
}
...
function corsConfiguration() {
    return (req, res, next) => {
        const allowedOrigins = env_1.CONFIG.CORS_ORIGIN ? env_1.CONFIG.CORS_ORIGIN.split(',') : [];
...
function validateInput(schema) {
    return async (req, res, next) => {
        try {
            const validated = await (0, input_validation_service_1.validate)(req.body, schema);
            req.body = validated;
            next();
        }
        catch (error) {
