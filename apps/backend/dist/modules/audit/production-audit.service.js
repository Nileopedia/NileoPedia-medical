...
exports.productionAuditService = exports.ProductionAuditService = void 0;
const client_1 = require("@prisma/client");
const knowledge_audit_service_1 = require("../medical/knowledge-audit.service");
const security_audit_service_1 = require("../security/security-audit.service");
...
class ProductionAuditService {
    constructor() {
        ...
        this.reportsDir = path_1.default.join(process.cwd(), 'production-reports');
        if (!fs_1.default.existsSync(this.reportsDir)) {
            fs_1.default.mkdirSync(this.reportsDir, { recursive: true });
        }
    }
    async runFullAudit() {
        logger_1.logger.info('Starting production audit...');
        ...
        try {
            logger_1.logger.info('Running knowledge audit...');
            report.sections.knowledgeAudit = await knowledge_audit_service_1.knowledgeAuditService.runAudit();
            logger_1.logger.info('Knowledge audit completed');
        }
        catch (error) {
        ...
        try {
            logger_1.logger.info('Running security audit...');
            report.sections.securityAudit = await security_audit_service_1.securityAuditService.runFullAudit();
            logger_1.logger.info('Security audit completed');
        }
        catch (error) {
        ...
        try {
            logger_1.logger.info('Checking health...');
            report.sections.healthCheck = await deployment_hardening_service_1.deploymentHardeningService.runHealthCheck();
        ...
        try {
            ...
            logger_1.logger.info('Configuration validation completed');
        }
        catch (error) {
        ...
        try {
            logger_1.logger.info('Validating database...');
            report.sections.databaseValidation = await this.validateDatabase();
            logger_1.logger.info('Database validation completed');
        }
        catch (error) {
            logger_1.logger.error(`Database validation failed: ${error}`);
            report.sections.databaseValidation = { error: error instanceof Error ? error.message : String(error) };
        }
        try {
            logger_1.logger.info('Validating Pinecone...');
            report.sections.pineconeValidation = await this.validatePinecone();
            logger_1.logger.info('Pinecone validation completed');
        }
        catch (error) {
            logger_1.logger.error(`Pinecone validation failed: ${error}`);
            report.sections.pineconeValidation = { error: error instanceof Error ? error.message : String(error) };
        }
        try {
            logger_1.logger.info('Validating Redis...');
            report.sections.redisValidation = await this.validateRedis();
            logger_1.logger.info('Redis validation completed');
        }
        catch (error) {
            logger_1.logger.error(`Redis validation failed: ${error}`);
            report.sections.redisValidation = { error: error instanceof Error ? error.message : String(error) };
        }
        try {
            logger_1.logger.info('Validating Groq...');
            report.sections.groqValidation = await this.validateGroq();
            logger_1.logger.info('Groq validation completed');
        }
        catch (error) {
            logger_1.logger.error(`Groq validation failed: ${error}`);
            report.sections.groqValidation = { error: error instanceof Error ? error.message : String(error) };
        }
        try {
            logger_1.logger.info('Validating Email...');
            report.sections.emailValidation = await this.validateEmail();
            logger_1.logger.info('Email validation completed');
        }
        catch (error) {
            logger_1.logger.error(`Email validation failed: ${error}`);
        ...
        logger_1.logger.info(`Production audit completed in ${totalDurationMs}ms. Score: ${overallScore}, Grade: ${grade}`);
        await this.writeReports(report);
        return report;
    }
    async validateEnvironment() {
        return {
            nodeEnv: env_1.CONFIG.NODE_ENV,
    ...
    async validateConfiguration() {
        const checks = {
            databaseConfigured: !!env_1.CONFIG.DATABASE_URL,
            jwtConfigured: !!env_1.CONFIG.JWT_ACCESS_SECRET && env_1.CONFIG.JWT_ACCESS_SECRET.length >= 32,
            pineconeConfigured: !!env_1.CONFIG.PINECONE_API_KEY,
            groqConfigured: !!env_1.CONFIG.GROQ_API_KEY,
            redisConfigured: !!env_1.CONFIG.REDIS_URL,
            emailConfigured: !!env_1.CONFIG.EMAIL_SERVICE_API_KEY,
            corsConfigured: !!env_1.CONFIG.CORS_ORIGIN,
        };
    ...
    async validatePinecone() {
        try {
            ...
            const indexName = env_1.CONFIG.PINECONE_INDEX_NAME || 'nileopedia';
            const indexExists = indexes.some((idx) => idx.name === indexName);
            return {
                connected: true,
                configured: !!env_1.CONFIG.PINECONE_API_KEY,
                indexName,
                indexExists,
                indexes: indexes.map((idx) => idx.name),
                success: !!env_1.CONFIG.PINECONE_API_KEY,
            };
        }
        catch (error) {
            return {
    ...
    async validateRedis() {
        try {
            ...
            const client = createClient({ url: env_1.CONFIG.REDIS_URL });
            ...
            return {
                configured: !!env_1.CONFIG.REDIS_URL,
                connected: false,
                error: error instanceof Error ? error.message : String(error),
                success: false,
            };
    ...
    async validateGroq() {
        try {
            ...
            return {
                configured: !!env_1.CONFIG.GROQ_API_KEY,
                connected: false,
                error: error instanceof Error ? error.message : String(error),
                success: false,
            };
    ...
    async validateEmail() {
        try {
            ...
            return {
                ...status,
                success: status.configured || (env_1.CONFIG.NODE_ENV === 'development'),
            };
        }
        catch (error) {
            return {
                configured: !!env_1.CONFIG.EMAIL_SERVICE_API_KEY,
                connected: false,
                error: error instanceof Error ? error.message : String(error),
                success: false,
            };
    ...
    extractCriticalIssues(report) {
        ...
        if (report.sections.securityAudit?.criticalIssues) {
            issues.push(...report.sections.securityAudit.criticalIssues.map((issue) => ({
                category: 'Security',
                ...issue,
            })));
        }
    ...
    extractWarnings(report) {
        ...
        if (report.sections.securityAudit?.warnings) {
            warnings.push(...report.sections.securityAudit.warnings.map((w) => ({
                category: 'Security',
                ...w,
            })));
        }
    ...
    async buildDeploymentChecklist() {
        const checklist = [
            {
                category: 'Security',
                items: [
            ...
            },
            {
                ...
                items: [
                    ...
                    { label: 'Redis connected', status: env_1.CONFIG.REDIS_URL ? 'PASS' : 'WARNING' },
                    { label: 'Groq connected', status: env_1.CONFIG.GROQ_API_KEY ? 'PASS' : 'WARNING' },
                ],
            },
            {
                category: 'Monitoring',
    ...
    calculateProductionReadiness(report) {
    ...
    calculateGoNoGo(grade, criticalIssues, warnings) {
    ...
    async writeReports(report) {
        ...
        const jsonPath = path_1.default.join(this.reportsDir, `${baseName}.json`);
        fs_1.default.writeFileSync(jsonPath, JSON.stringify(report, null, 2));
        logger_1.logger.info(`JSON report written to ${jsonPath}`);
        const markdownPath = path_1.default.join(this.reportsDir, `${baseName}.md`);
        fs_1.default.writeFileSync(markdownPath, this.generateMarkdownReport(report));
        logger_1.logger.info(`Markdown report written to ${markdownPath}`);
        const htmlPath = path_1.default.join(this.reportsDir, `${baseName}.html`);
        fs_1.default.writeFileSync(htmlPath, this.generateHTMLReport(report));
        logger_1.logger.info(`HTML report written to ${htmlPath}`);
        const latestJsonPath = path_1.default.join(this.reportsDir, 'production-audit-latest.json');
        fs_1.default.writeFileSync(latestJsonPath, JSON.stringify(report, null, 2));
        const latestMarkdownPath = path_1.default.join(this.reportsDir, 'production-audit-latest.md');
        fs_1.default.writeFileSync(latestMarkdownPath, this.generateMarkdownReport(report));
    }
...
exports.ProductionAuditService = ProductionAuditService;
exports.productionAuditService = new ProductionAuditService();
//# sourceMappingURL=production-audit.service.js.map