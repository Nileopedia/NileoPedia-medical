...
const medical_documents_helper_1 = require("./helpers/medical-documents.helper");
...
describe('E2E: Medical Document Ingestion Validation', () => {
    ...
    let adminId;
    beforeAll(async () => {
        await (0, prisma_helper_1.cleanupDatabase)();
        const user = await (0, test_helpers_1.createTestUser)({ email: `e2e-medical-${Date.now()}@example.com`, role: 'MEDICAL_USER' });
        userId = user.id;
        authToken = (0, test_helpers_1.generateTestJWT)({ id: user.id, email: user.email, role: user.role });
        const admin = await (0, test_helpers_1.createTestUser)({ email: `e2e-admin-${Date.now()}@example.com`, role: 'ADMIN' });
        adminId = admin.id;
        adminToken = (0, test_helpers_1.generateTestJWT)({ id: admin.id, email: admin.email, role: admin.role });
    });
    afterAll(async () => {
        await (0, medical_documents_helper_1.cleanupTestDocuments)();
        await (0, prisma_helper_1.disconnectPrisma)();
    });
    afterEach(async () => {
        await (0, medical_documents_helper_1.cleanupTestDocuments)();
    });
    describe('1. Document Upload and Ingestion', () => {
        it('should upload and ingest a hypertension guideline document', async () => {
            const fixture = medical_documents_helper_1.MEDICAL_DOCUMENTS[0];
            const document = await (0, medical_documents_helper_1.createTestDocument)(fixture, userId);
        ...
        it('should ingest diabetes mellitus document', async () => {
            const fixture = medical_documents_helper_1.MEDICAL_DOCUMENTS[1];
            const document = await (0, medical_documents_helper_1.createTestDocument)(fixture, userId);
            const response = await (0, supertest_1.default)(app_1.app)
                .get(`/api/v1/documents/${document.id}`)
                .set('Authorization', `Bearer ${authToken}`);
    ...
    describe('2. Metadata Extraction Verification', () => {
        it('should extract correct metadata for hypertension document', async () => {
            ...
            const metadata = await prisma_helper_1.prisma.documentMetadata.findUnique({
        ...
        it('should extract ICD-10 codes', async () => {
            ...
            if (metadata && metadata.icd10) {
                const hasHypertensionCode = metadata.icd10.startsWith('I10') ||
    ...
    describe('3. Chunk Generation Verification', () => {
        it('should generate chunks for ingested document', async () => {
            const fixture = medical_documents_helper_1.MEDICAL_DOCUMENTS[0];
            const document = await (0, medical_documents_helper_1.createTestDocument)(fixture, userId);
            const chunks = await prisma_helper_1.prisma.embeddingMetadata.findMany({
                where: { documentId: document.id }
            });
        ...
        it('should have low duplicate rate', async () => {
            const fixture = medical_documents_helper_1.MEDICAL_DOCUMENTS[0];
            const document = await (0, medical_documents_helper_1.createTestDocument)(fixture, userId);
            const total = await prisma_helper_1.prisma.embeddingMetadata.count({
                where: { documentId: document.id }
            });
            const duplicates = await prisma_helper_1.prisma.embeddingMetadata.count({
    ...
    describe('6. Retrieval Quality Verification', () => {
        it('should retrieve hypertension document via semantic search', async () => {
            ...
            if (response.status === 200 && response.body.success) {
                ...
                if (results.length > 0) {
                    const hasHypertension = results.some((r) => (r.metadata?.text || r.metadata?.textPreview || '').toLowerCase().includes('hypertension'));
    ...
    describe('7. AI Response Quality Verification', () => {
        it('should generate AI response with citations', async () => {
            const questionResponse = await (0, supertest_1.default)(app_1.app)
            ...
            if (questionResponse.status === 201 || questionResponse.status === 200) {
                ...
                if (questionId) {
                    ...
                    const question = await prisma_helper_1.prisma.question.findUnique({
                        where: { id: questionId },
    ...
    describe('8. Multi-Document Ingestion', () => {
        it('should ingest hypertension and diabetes documents', async () => {
            const hypertensionDoc = await (0, medical_documents_helper_1.createTestDocument)(medical_documents_helper_1.MEDICAL_DOCUMENTS[0], userId);
            const diabetesDoc = await (0, medical_documents_helper_1.createTestDocument)(medical_documents_helper_1.MEDICAL_DOCUMENTS[1], userId);
            const htnChunks = await prisma_helper_1.prisma.embeddingMetadata.count({
                where: { documentId: hypertensionDoc.id }
            });
            const dmChunks = await prisma_helper_1.prisma.embeddingMetadata.count({
        ...
        it('should have distinct chunks for different documents', async () => {
            const doc1 = await (0, medical_documents_helper_1.createTestDocument)(medical_documents_helper_1.MEDICAL_DOCUMENTS[0], userId);
            const doc2 = await (0, medical_documents_helper_1.createTestDocument)(medical_documents_helper_1.MEDICAL_DOCUMENTS[1], userId);
            const chunks1 = await prisma_helper_1.prisma.embeddingMetadata.findMany({
                where: { documentId: doc1.id },
                select: { chunkText: true }
            });
            const chunks2 = await prisma_helper_1.prisma.embeddingMetadata.findMany({
            ...
            const hasDistinctContent = text1.includes('hypertension') || text2.includes('diabetes');
    ...
    describe('10. Complete Workflow Validation', () => {
        it('should complete full workflow: upload -> process -> retrieve -> ask', async () => {
            ...
            const document = await (0, medical_documents_helper_1.createTestDocument)(fixture, userId);
            await new Promise(resolve => setTimeout(resolve, 3000));
            const docResponse = await (0, supertest_1.default)(app_1.app)
                .get(`/api/v1/documents/${document.id}`)
                .set('Authorization', `Bearer ${authToken}`);
...
//# sourceMappingURL=medical-document-ingestion.e2e.js.map