...
exports.MEDICAL_DOCUMENTS = exports.cleanupTestDocuments = exports.createTestDocument = void 0;
...
exports.MEDICAL_DOCUMENTS = MEDICAL_DOCUMENTS;
async function createTestDocument(fixture, userId) {
    const uploadDir = path_1.default.join(process.cwd(), 'tests', 'e2e', 'fixtures');
    if (!fs_1.default.existsSync(uploadDir)) {
        fs_1.default.mkdirSync(uploadDir, { recursive: true });
    }
    const fileName = `e2e-${Date.now()}-${Math.random().toString(36).substring(2, 9)}.${fixture.fileType}`;
    const filePath = path_1.default.join(uploadDir, fileName);
    fs_1.default.writeFileSync(filePath, fixture.content);
...
async function cleanupTestDocuments() {
    const fixtureDir = path_1.default.join(process.cwd(), 'tests', 'e2e', 'fixtures');
    if (fs_1.default.existsSync(fixtureDir)) {
        fs_1.default.rmSync(fixtureDir, { recursive: true, force: true });
    }
}
exports.cleanupTestDocuments = cleanupTestDocuments;
//# sourceMappingURL=medical-documents.helper.js.map