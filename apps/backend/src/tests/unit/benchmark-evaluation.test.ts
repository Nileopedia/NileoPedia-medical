...
interface BenchmarkResult {
  ...
  checks: {
    ...
    acronymExpanded: boolean;
    relevantChunks: boolean;
    noUnrelated: boolean;
  ...
  BENCHMARK_QUERIES.forEach((benchmarkQuery) => {
    describe(`Query: "${benchmarkQuery.query}"`, () => {
      it('should expand synonyms', async () => {
        const expansion = medicalSynonymService.expand(benchmarkQuery.query);
      ...
      it('should expand acronyms when applicable', async () => {
        const expansion = medicalAcronymResolver.resolveAll(benchmarkQuery.query);
      ...
      it('should have dynamic retrieval weights', async () => {
        const analysis = dynamicRetrievalService.analyzeQuery(benchmarkQuery.query);
