...
import { productionAuditService } from '../modules/audit/production-audit.service';
...
async function main() {
  ...
  try {
    const report = await productionAuditService.runFullAudit();
    ...
    if (report.criticalIssues.length > 0) {
      console.log('\nCRITICAL ISSUES:');
      report.criticalIssues.forEach((issue: any, index: number) => {
    ...
    if (report.warnings.length > 0) {
      console.log('\nWARNINGS:');
      report.warnings.forEach((warning: any, index: number) => {
    ...
    logger.error('Production audit failed', error);
    process.exit(1);
  }
}
