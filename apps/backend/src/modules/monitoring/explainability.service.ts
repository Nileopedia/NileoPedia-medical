...
export class ExplainabilityService {
  ...
  private generateSelectionReason(doc: {
    similarityScore: number;
    rerankerScore: number;
    citationQuality: number;
    metadataCompleteness: number;
  ...
  private getEvidenceLevel(citationQuality: number, specialty: string): string {
    if (citationQuality >= 9) return 'Level I - Systematic Review/Meta-Analysis';
    if (citationQuality >= 8) return 'Level II - Randomized Controlled Trial';
    if (citationQuality >= 7) return 'Level III - Cohort Study';
    if (citationQuality >= 6) return 'Level IV - Case-Control Study';
    if (citationQuality >= 5) return 'Level V - Expert Opinion';
  ...
  formatExplanationForUI(explanation: Explanation): string {
    ...
    for (const [key, value] of Object.entries(explanation.confidenceBreakdown)) {
      output += `- ${key}: ${(value as number).toFixed(2)}\n`;
    }
