...
export class QualityMonitoringService {
  ...
  async collectMetrics(): Promise<QualityMetric[]> {
  ...
  private async collectKnowledgeCoverage(timestamp: Date): Promise<QualityMetric> {
    try {
  ...
  private async collectRetrievalPrecision(timestamp: Date): Promise<QualityMetric> {
    try {
      const recentQueries = await prisma.question.findMany({
  ...
  private async collectContextRecall(timestamp: Date): Promise<QualityMetric> {
    try {
  ...
  private async collectCitationQuality(timestamp: Date): Promise<QualityMetric> {
    try {
      const responses = await prisma.aIResponse.findMany({
        where: { createdAt: { gte: new Date(Date.now() - 24 * 60 * 60 * 1000) } },
  ...
  private async collectHallucinationRate(timestamp: Date): Promise<QualityMetric> {
    try {
      ...
      const hallucinations = reviews.filter(r => 
        r.feedback?.toLowerCase().includes('hallucination') ||
        r.feedback?.toLowerCase().includes('incorrect')
      ).length;
  ...
  private async collectDuplicateChunkRate(timestamp: Date): Promise<QualityMetric> {
    try {
  ...
  private async collectAverageChunkSize(timestamp: Date): Promise<QualityMetric> {
    try {
  ...
  private async collectEmbeddingFailures(timestamp: Date): Promise<QualityMetric> {
    try {
      const failures = await prisma.medicalDocument.count({
  ...
  private async collectQueueFailures(timestamp: Date): Promise<QualityMetric> {
  ...
  private async collectUploadFailures(timestamp: Date): Promise<QualityMetric> {
    try {
      const failures = await prisma.medicalDocument.count({
        where: { ingestionStatus: 'FAILED' }
      });
  ...
  private async collectTopFailedQueries(timestamp: Date): Promise<QualityMetric> {
    try {
      ...
      const failedQuestions = questions.filter(q => 
        q.aiResponse && q.aiResponse.validationStatus === 'REJECTED'
      );
  ...
  private async collectMostCommonDiseases(timestamp: Date): Promise<QualityMetric> {
    try {
      const audit = await knowledgeAuditService.runAudit();
  ...
  private async collectMostAccessedDocuments(timestamp: Date): Promise<QualityMetric> {
    try {
      const responses = await prisma.aIResponse.findMany({
        where: { createdAt: { gte: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) } },
        include: { citations: true },
  ...
  private async collectDeadDocuments(timestamp: Date): Promise<QualityMetric> {
    try {
      const deadDocs = await prisma.medicalDocument.count({
        where: {
  ...
  private async collectUnusedDocuments(timestamp: Date): Promise<QualityMetric> {
    try {
      const unusedDocs = await prisma.medicalDocument.count({
        where: {
          ingestionStatus: 'COMPLETED',
  ...
  private async collectExpiredDocuments(timestamp: Date): Promise<QualityMetric> {
    try {
      const expiredDocs = await prisma.medicalDocument.count({
        where: {
  ...
  private async collectBrokenCitations(timestamp: Date): Promise<QualityMetric> {
    try {
      ...
      const broken = citations.filter(c => 
        !c.title || c.title === 'Unknown' || !c.source || c.source === 'Unknown'
      ).length;
  ...
  private async collectAverageConfidence(timestamp: Date): Promise<QualityMetric> {
    try {
      const responses = await prisma.aIResponse.findMany({
        where: { 
      ...
      const scores = responses.map(r => r.confidenceScore as number);
  ...
  private async collectAverageEvidenceLevel(timestamp: Date): Promise<QualityMetric> {
    try {
      const responses = await prisma.aIResponse.findMany({
        where: { createdAt: { gte: new Date(Date.now() - 24 * 60 * 60 * 1000) } },
        take: 50,
      ...
      const evidenceLevels = responses.map(r => {
        if (r.documentsUsed && r.documentsUsed >= 5) return 'Very High';
        if (r.documentsUsed && r.documentsUsed >= 3) return 'High';
        if (r.documentsUsed && r.documentsUsed >= 2) return 'Moderate';
        return 'Low';
      });
  ...
  private async collectGroqTokenUsage(timestamp: Date): Promise<QualityMetric> {
    try {
      const responses = await prisma.aIResponse.findMany({
        where: { createdAt: { gte: new Date(Date.now() - 24 * 60 * 60 * 1000) } },
  ...
  private async collectPineconeUsage(timestamp: Date): Promise<QualityMetric> {
    try {
      const totalVectors = await prisma.embeddingMetadata.count();
  ...
  private async collectStorageGrowth(timestamp: Date): Promise<QualityMetric> {
    try {
      const docs = await prisma.medicalDocument.count();
      const chunks = await prisma.embeddingMetadata.count();
      const responses = await prisma.aIResponse.count();
  ...
  private checkAlerts(metrics: QualityMetric[]): void {
  ...
  private updateTrends(metrics: QualityMetric[]): void {
    metrics.forEach(metric => {
