...
export class KnowledgeGapDetectionService {
  private queryHistory: Map<string, { count: number; lastSearched: Date; hasResults: boolean }> = new Map();
  ...
  async detectGaps(): Promise<KnowledgeGapReport> {
    const gaps: KnowledgeGapAlert[] = [];
    const recommendations: Set<string> = new Set();

    const diseaseQueries = new Map<string, { count: number; lastSearched: Date; hasResults: boolean }>();
    const symptomQueries = new Map<string, { count: number; lastSearched: Date; hasResults: boolean }>();
  ...
  private suggestDocuments(query: string): string[] {
    ...
    if (suggestions.length === 0) {
      suggestions.push(
        `WHO ${query} Guideline`,
        `${query} Clinical Practice Guidelines`,
        `NIH ${query} Evidence Report`
      );
    }
  ...
  getQueryHistory(): Array<{ query: string; count: number; lastSearched: Date; hasResults: boolean }> {
...
export const knowledgeGapDetectionService = new KnowledgeGapDetectionService();