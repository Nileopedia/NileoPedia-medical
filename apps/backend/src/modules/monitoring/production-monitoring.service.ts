...
export class ProductionMonitoringService {
  ...
  getSystemMetrics(timeRangeMs = 3600000): SystemMetrics {
  ...
  getMetricsByMetricName(metricName: string, timeRangeMs = 3600000): MonitoringMetric[] {
    const cutoff = new Date(Date.now() - timeRangeMs);
  ...
  async getDashboardData(): Promise<{
    ...
    missingDiseases: Array<{ disease: string; searchCount: number }>;
    failedSearches: Array<{ query: string; error: string; timestamp: Date }>;
  }> {
    ...
    const failedSearches: Array<{ query: string; error: string; timestamp: Date }> = [];
    ...
    const missingDiseases = Array.from(searchCounts.entries())
      .filter(([query]) => {
        const hasResults = recentQueries.some(q => 
          q.query === query && q.success && q.chunkCount > 0
        );
        return !hasResults;
      })
