...
export class CitationQualityService {
  private readonly sourceTiers: Record<string, { tier: CitationQualityResult['tier']; weight: number }> = {
  ...
  getTierPriority(tier: CitationQualityResult['tier']): number {
    const priorities: Record<CitationQualityResult['tier'], number> = {
      'WHO': 1,
      'CDC': 2,
      'NIH': 3,
      'NICE': 4,
...
export const citationQualityService = new CitationQualityService();
